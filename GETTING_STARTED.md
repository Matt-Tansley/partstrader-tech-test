...
Notable files and folders include:
...
As noted above, I have the below versions of NodeJS and NPM on my local machine.
...
- UI test cases 14 and 15 share many of the same webpages. To avoid code duplication it is best practice to use the page object model pattern.
- All the test cases (UI and API) use some shared settings, such as base URL.
...
Config has been created to dynimically switch between environments. In this case, we have dev and test environments. Different environment variables for these are defined under the `env/` folder. For the sake of this exercise, configuring environment is rudimentary and gets set based on the `ENV` environment variable. By default (i.e if `ENV` is not set) then the dev environment is used.

**Improvements:**

- [ ] Passwords and other test data could be considered sensitive data, even in non-prod environments. Better ways to store sensitve data could be in separate `.env` files which are git ignored but can be used locally, and GitHub Actions can use secrets that the workflow can securely access.
...
- Base URL has been set in `playwright.config.ts` as https://automationexercise.com, and is set based on an environment variable.
...
- For adding products to the cart, there are multiple ways to navigate to the Products page, and these could be covered under separate tests. The main goal of Test Case 14 and 15 is to test register and checkout, and it just so happens that products need to be in the cart to achieve this, but the test cases care less about how the products ended up in the cart. Ultimately: requirements should be clarified with stakeholders to understand what they would like to see from the test case, and new test cases should be created if needed.
- For test case 14; there is an assumption that the user is logged out initially. This results in a different user view when clicking 'proceed to checkout' (not logged in a model appears, if logged in the user is navigated to a new page). Therefore I have written basic logic to handle which page is the 'current page' in the test code. How would you handle this?
...
- With Playwright, normally we'd use the 'data' option when sending request payloads, however because the system API has been implemented with x-www-form-urlencoded content type, data needs to be sent as form data. Playwright does has a 'form' option, and this appears to automatically set the x-www-form-urlencoded content type header, so setting this at the start of the API tests could be removed.
...
- [ ] Seed for random test data could be configurable, so we can dynamically provide a seed.
...
- [ ] For Test Case 14, step 18, the 'Your order has been placed successfully!' message only briefly shows for a few seconds. My test is currently verifying a message that shows up on the subsequent page, need to address this, and clarify the test requirement.
- [ ] For sign up and credit card forms, there should be other tests for validating the forms and testing edge cases.
...
- [ ] Account created page success message is hardcoded twice in the ui tests. If the message changes, then code needs to be updated in more than 1 place. Improvement would be to refactor this so that the success message is defined once (perhaps return as a method call to account created page). Similar improvements could be made to all the page classes where specific text is expected on the screen.
- [ ] Parts of Test Case 14 and 15 could be abstracted into functions. The test cases share much of the same functionality, and so this shared functionality could be refactored into reusable functions (I have done this for test data creation, but it could be done further). For now, since there are only 2 test cases it is manageable, however as the test suite grows it may be easier to maintain.
